...
class WsPool:
    ...
    async def _initialize_dummy_connection(self):
        ...
        dummy_connection = await websocket_factory(self.state.ws_uri, extra_headers=self.auth.get_auth_headers_ws())
    ...
    async def _create_connections_(self):
        ...
        tasks = [websocket_factory(self.state.ws_uri, extra_headers=self.auth.get_auth_headers_ws()) for _ in range(self.n_connections)]
        ...
        self.connections = {conn_id: conn for conn_id, conn in zip(connection_ids, connections)}
    ...
    async def _measure_all_pings_(self):
        start_times = {conn_id: time.perf_counter() for conn_id in self.connections.keys()}
        tasks = [self._ping_connection_(conn_id) for conn_id in self.connections.keys()]
        await asyncio.gather(*tasks)
        latencies = {conn_id: time.perf_counter() - start_times[conn_id] for conn_id in self.connections.keys()}
    ...
    async def monitor(self):
        ...
        tasks = [asyncio.create_task(ping_task(connection_id)) for connection_id in self.connections.keys()]
        await asyncio.gather(*tasks)
